...
#
#	$Id: maketoolbar.py 93 2005-10-11 02:51:02Z limodou $
...
def maketoolbar(win, toollist, toolbaritems):
	...
	win.toolbar.SetToolBitmapSize(size)

	toollist.sort()
	debug.info('[maketoolbar] toolbar list...')
	for order, name in toollist:
		if name == '|':
			...
			if style == wx.ITEM_NORMAL:
				win.toolbar.AddSimpleTool(id, image, shorttext, longtext)
			elif style == wx.ITEM_CHECK:
				win.toolbar.AddCheckTool(id, image, shortHelp=shorttext, longHelp=longtext)
#			elif style == wx.ITEM_RADIO:
#				win.toolbar.AddRadioTool(id, image, shortHelp=shorttext, longHelp=longtext)
			else:
