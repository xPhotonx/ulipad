...
#
#	$Id: SnippetWindow.py 176 2005-11-22 02:46:37Z limodou $
...
class MySnippetCatalog(wx.TreeCtrl):
	def __init__(self, parent, mainframe, pref):
		wx.TreeCtrl.__init__(self, parent, -1, style = wx.TR_SINGLE|wx.TR_HAS_BUTTONS)
		self.parent = parent
		self.pref = pref
		self.mainframe = mainframe

		imagelist = mainframe.snippet_imagelist
		_imagel = wx.ImageList(16, 16)
		_imagel.Add(wx.Image(imagelist['close']).ConvertToBitmap())
		_imagel.Add(wx.Image(imagelist['open']).ConvertToBitmap())
		_imagel.Add(wx.Image(imagelist['item']).ConvertToBitmap())
		self.snippetsimagelist = _imagel

		self.close_image = 0
		self.open_image = 1
		self.item_image = 2

		self.SetImageList(self.snippetsimagelist)
...
class SnippetsCatalogDialog(wx.Dialog):
	...
	def OnIdle(self, event):
		...
		self.obj_ID_ADDSUBITEM.Enable(catalog_selected)
		self.obj_ID_UP.Enable(catalog_selected)
		self.obj_ID_DOWN.Enable(catalog_selected)
		self.obj_ID_LEFT.Enable(catalog_selected)
		self.obj_ID_RIGHT.Enable(catalog_selected)
		self.obj_ID_DELETE.Enable(catalog_selected)
		self.obj_ID_UPDATE.Enable(catalog_selected)
...
def getFilename(mainframe, id):
	if id:
		return os.path.join(mainframe.app.workpath, 'snippets/snippet%d.xml' % id)
	else:
		return ''