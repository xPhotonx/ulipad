...
#
#	$Id: HtmlPage.py 93 2005-10-11 02:51:02Z limodou $
...
class HtmlPage(wx.Panel, DocumentBase.DocumentBase, Mixin.Mixin):
	...
	def __init__(self, parent, mainframe, filename, documenttype):
		...
		self.btnEdit = wx.Button(self, self.ID_EDIT, tr("Edit"), size=(60, 22))
		subbox.Add(self.btnEdit, 0, wx.RIGHT, 2)

		self.ID_BACK = wx.NewId()
		self.btnBack = wx.Button(self, self.ID_BACK, "<", size=(22, 22))
		subbox.Add(self.btnBack, 0, wx.RIGHT, 2)

		self.ID_FORWARD = wx.NewId()
		self.btnForward = wx.Button(self, self.ID_FORWARD, ">", size=(22, 22))
	...
	def OnEdit(self, event):
		for document in self.mainframe.editctrl.list:	#if the file has been opened
			if document.isMe(self.filename, documenttype = 'edit'):
