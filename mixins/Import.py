...
#
#	$Id: Import.py 111 2005-10-16 10:10:27Z limodou $
...
def OnDocumentTabIndent(win, event):
	...
	win.SetUseTabs(win.document.usetab)
...
class AboutDialog(wx.Dialog):
	def __init__(self, parent):
		...
		t.SetFont(font)
		box.Add(t, 0, wx.ALIGN_CENTER|wx.BOTTOM, 10)
		btnOK = wx.Button(self, wx.ID_OK, tr("OK"), size=(60, 20))
		btnOK.SetDefault()
...
menulist = [('IDM_TOOL',
	[
		(110, 'IDM_SCRIPT', tr('Script'), wx.ITEM_NORMAL, None, ''),
	]),
...
lexer = [(LexerClass.CLexer.metaname, tr('C/C++|*.c;*.cpp;*.h;*.cxx;*.hpp'), wx.stc.STC_LEX_CPP, common.unicode_abspath('c.stx'), LexerClass.CLexer)]
Mixin.setMixin('lexerfactory', 'lexers', lexer)

lexer = [(LexerClass.TextLexer.metaname, 'Text|*.txt;*.bak', wx.stc.STC_LEX_NULL, common.unicode_abspath('text.stx'), LexerClass.TextLexer)]
Mixin.setMixin('lexerfactory', 'lexers', lexer)

lexer = [(LexerClass.HtmlLexer.metaname, tr('Html|*.htm;*.html;*.shtml;*.xml;*.xslt'), wx.stc.STC_LEX_HTML, common.unicode_abspath('html.stx'), LexerClass.HtmlLexer)]
Mixin.setMixin('lexerfactory', 'lexers', lexer)

lexer = [(LexerClass.PythonLexer.metaname, tr('Python|*.py;*.pyw'), wx.stc.STC_LEX_PYTHON, common.unicode_abspath('python.stx'), LexerClass.PythonLexer)]
Mixin.setMixin('lexerfactory', 'lexers', lexer)
...
def OnChar(win,event):
	...
	if chr(key) in ['(',')','.'] and not control and not alt:
		...
		# GF correspond to '(', ')' or '.'
		event.Skip()
Mixin.setMixin('editor', 'OnChar', OnChar)
