...
class BlogManageWindow(wx.Panel, Mixin.Mixin):
	...
	def getposts(self, number=1, postid=''):
		setmessage(self.mainframe, tr('Getting entries...'))
		...
		try:
			...
			for i, entry in enumerate(result):
				...
				if not data['postid']:
					pos = data['guid'].rfind('#')
					data['postid'] = categories[0] + '/' + data['guid'][pos+1:]
	...
	def delete(self, postid=''):
		if not postid:
			index = self.list.GetNextItem(-1, wx.LIST_NEXT_ALL, wx.LIST_STATE_SELECTED)
			if index == -1:
				showmessage(self.mainframe, tr('You should select on entry first!'))
				return
			filename = self.data[index]
			tree = Tree()
		...
		setmessage(self.mainframe, tr('Deleting entry...'))
		site = self.pref.blog_sites_info[self.pref.blog_sites[self.cmbSite.GetSelection()]]
		try:
			server = xmlrpclib.ServerProxy(site['url'])
			result = server.blogger.deletePost('', postid, site['user'], site['password'], False)
			if result:
				showmessage(self.mainframe, tr('Delete is successful!'))
				self.list.DeleteItem(index)
			else:
				showerror(self.mainframe, tr('Delete error!'))
	...
	def login(self):
		try:
			...
			if result:
				...
				showmessage(self.mainframe, tr('Login is successful!'))
			else:
				self.categories = {}
				showerror(self.maiframe, tr('Login is faild!'))
			return True
		except Exception, msg:
			error.traceback()
			showerror(self.mainframe, msg)
